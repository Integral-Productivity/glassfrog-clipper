...
reporter could go red without blocking a merge. #88 therefore made
`Software Fitness / Self-compliance` required first (ADR 0012), and only then
deleted the duplicate. One implementation is what makes two reporters agree;
being required is what makes a reporter a gate.
