...
- `aShareWithNoUrlAttachmentStillFilesTheRest` (`:419`) — **was named `aStalledAttachmentStillFilesTheRest`, and did not do what that name said. It is the reason to check citations by reading them.** Its only attachment is `textProvider` (`:42-44`), which answers immediately; `silentProvider()` has exactly one call site in the file and it is `:373`, above. What the test pins is that a share with no URL attachment still files its title and selection — worth asserting, but `load` is never called for the URL slot, so the deadline path is untouched. It would pass unchanged with the deadline machinery deleted. Its own comment made the same claim its name did, so the error was in the test, not in the reading of it.

  Renamed under **#167**, which stays open for the behaviour the old name claimed: a genuine stall alongside an attachment that arrives. That needs a deadline seam on `pageContext(from:)` — which today takes the production ten-second `loadDeadline` (`:106`) rather than an injected one — and the acceptance is a mutation check on a macOS runner, so it is not work a Linux session can honestly close. The name no longer advertises coverage that does not exist, which was the costly half: a reader auditing the stall path found this test, read the name, and stopped looking.
